...
export default function PortfolioHome() {
  ...
  const handleSendMessage = async (text: string, category?: string) => {
    ...
    setTimeout(() => {
      ...
      const aiMsg: Message = { id: aiId, role: 'ai', text: responseText, type: messageType, suggestions };
  ...
  const handleMenuClick = (label: string) => {
    ...
    setTimeout(() => handleSendMessage(question, label), 200);
  };
        ...
        <div className="relative z-10 flex flex-col h-[100dvh] max-w-5xl mx-auto">
          ...
          >
            <div className="flex flex-col items-center">
              ...
              {hasMessages && (
                 <motion.h3 initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-sm font-bold text-slate-800 dark:text-neutral-200 mt-1">
                   Arham's Work
                 </motion.h3>
              )}
            </div>
          ...
          {hasMessages && (
            <div className="flex-1 overflow-y-auto px-4 py-6 scroll-smooth w-full">
              <div className="max-w-4xl mx-auto">
                {messages.map((msg) => (
                  <ChatBubble key={msg.id} message={msg} onActionClick={handleMenuClick} />
                ))}
                
                {isTyping && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex justify-start mb-8">
          ...
          <div className="shrink-0 w-full px-4 pb-4 md:pb-6 z-30">
            <div className="max-w-3xl mx-auto flex flex-col gap-6">
              ...
              <div className="w-full bg-white dark:bg-black rounded-full border border-slate-200 dark:border-neutral-800 shadow-sm px-2 py-2 flex items-center transition-all focus-within:shadow-md focus-within:border-blue-300 dark:focus-within:border-blue-700">
                ...
                />
                <button onClick={() => handleSendMessage(inputValue)} disabled={!inputValue.trim()} className={cn("w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 shrink-0", inputValue.trim() ? "bg-[#3B82F6] text-white shadow-md hover:scale-105 active:scale-95" : "bg-slate-100 dark:bg-neutral-900 text-slate-300 dark:text-neutral-700 cursor-not-allowed")}>
              ...
              <div className="flex gap-2 overflow-x-auto pb-4 pt-1 no-scrollbar mask-fade-sides justify-start md:justify-center px-1">
                {MENU_ITEMS.map((item) => (
                  <ActionChip key={item.id} icon={item.icon} label={item.label} onClick={() => handleMenuClick(item.label)} />
                ))}
              </div>
